/*----------------------------------------------------------------------
	...
	---------------------------------------------------------------------------*/
// CORREGIDA LA FUNCION LCD_FLOAT POR JOSE MANUEL REQUENA. ANTES NO MOSTRABA LOS DECIMALES.
#include "LCD4bits.h"
#include "stdio.h"
...
void LCD_Float(float valor)
{
	...
	char cadena_float[16];
        sprintf(cadena_float, "%.2f", valor); // Convierte el float a una cadena de caracteres
...
void Lstart(){
    LCD_INI("0","character");
    LCD_Cadena("Pulsa el Boton");
    LCD_FilaColumna("0","1");
    LCD_Cadena("Para comenzar");
    return;
}
void Lgameover(){
    LCD_INI("0","character");
