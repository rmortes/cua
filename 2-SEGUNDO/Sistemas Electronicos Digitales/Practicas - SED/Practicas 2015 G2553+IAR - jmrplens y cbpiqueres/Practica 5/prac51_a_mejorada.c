...
int main(void)
{
  WDTCTL = WDTPW + WDTHOLD;         // Apagamos el Watch dog y ponemos a 1 el WDTNMI para activar la patilla RST/NMI como NMI
  config_port();
  config_screen();
  LCD_Control(ClearDisplay);        // Borra el display
  LCD_Cadena ("Esperando...");      // Imprime esperando
  __enable_interrupt() ;
  __bis_SR_register(LPM0_bits);     // Bajo consumo, el MSP se para hasta que salga del modo bajo consumo
