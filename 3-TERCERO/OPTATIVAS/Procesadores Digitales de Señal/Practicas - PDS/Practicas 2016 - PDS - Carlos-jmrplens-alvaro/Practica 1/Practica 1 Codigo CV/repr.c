...
int main() {
    ...
    if ((err = snd_pcm_open(&snd_handle, "hw:0,0", SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
    ...
    while (fread(buf, sizeof(char), BUFFER_SIZE, stdin) != 0) {
        if ((err = snd_pcm_writei(snd_handle, buf, 64)) != 64) {
