...
//gcc -o fullduplex practica1_fullduplex.c -lasound -lm
...
void effect(unsigned char *buf, int numMuestras) {
    ...
    for (i = 0; i < numMuestras * 4; i += 4) {
        ...
        /****************** TONO *********************/
		muestraR = 5000*sin(2*3.14*fc/gRate);
		muestraL = 5000*sin(2*3.14*fc/gRate);	
        /***********************************************/
...
int gBufferSize = 2;
		...
		//PITIDO
		//muestraR = 5000*cos(2*3.14*fc/gRate);
		//muestraL = 5000*cos(2*3.14*fc/gRate);	
//Funcion donde configuraremos los parametros que vamos a utilizar durante el programa y que nos indique si hay algun error en la configuracion, para poder solucioanrlo.
//Si se detecta algun error, se devolvera la variable err, que activara un if en la funcion main, y aparecera el mensaje de error.
